...
struct MeasurementEditorV: View {
    ...
    var body: some View {
        GeometryReader { _ in
            ...
            .onChange(of: backgroundMapDetails.position, initial: true) { recalculateHandlePositions() }
            .onChange(of: startingPos, initial: true) { calculateDistance() }
            .onChange(of: endingPos) { calculateDistance() }
        }
    }
    
    /// Uses the map context to determine where the handle positions are supposed to be.
    private func recalculateHandlePositions() {
        ...
        self.startingPos = CGSize(cgPoint: startingPos)
        self.endingPos = CGSize(cgPoint: endingPos)
        calculateDistance()
    }
