...
struct PhotoEditorCompositeV: View {
    ...
    init(mapMap: MapMap) {
        self.mapMap = mapMap
        if let corners = mapMap.cropCorners { self._handleTracker = State(initialValue: HandleTrackerM(stockCorners: FourCornersStorage(corners: corners))) }
        else { self._handleTracker = State(initialValue: HandleTrackerM(stockCorners: FourCornersStorage(fill: mapMap.imageDefault?.size ?? .zero))) }
